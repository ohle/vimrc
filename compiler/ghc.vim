...
" show hoogle info for identifier under curser
" --joc
map <LocalLeader>d :call GHC_ShowDoc()<cr>
function! GHC_ShowDoc()
  let namsym   = haskellmode#GetNameSymbol(getline('.'),col('.'),0)
  if namsym==[]
    redraw
    echo 'no name/symbol under cursor!'
  ...
  let name  = qual=='' ? unqual : qual.'.'.unqual
  let pname = ( symb ? '('.name.')' : name ) 
  "let doc = system('hoogle --info ' + pname)
  echo system('hoogle --info ' . pname)
endfunction
